...
# calculate total score for pcl5
...
# put total score into the df

len_df <- length(df$pcl5total)

len_data = length(pcl5total)

num_nans <- len_df - len_data

nan_vec <- rep(NA, num_nans)

full_vec = c(nan_vec, pcl5total)

df$pcl5total <- full_vec